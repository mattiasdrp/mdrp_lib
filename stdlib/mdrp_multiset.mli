module Make (X : sig
  ...
  type t
  (** The immutable type of multisets. *)
  ...
  val occ : elt -> t -> int
  (** [occ x b] is the number of occurrences of [x] in multiset [b].
  ...
  (** [add x ?mult b] returns a new multiset where the multiplicity of [x]
      is increased by [mult] (defaults to one).
      Raises [Invalid_argument] if [mult] is negative.*)

  val update : elt -> (int -> int) -> t -> t
  (** [update x f b] returns a multiset containing the same elements as
      [b], except for element [x] whose multiplicity is [f (occ x b)].
      Raises [Invalid_argument] if [f] returns a negative value. *)

  val singleton : elt -> t
  (** [singleton x] is a multiset with one element [x], with multiplicity [1]. *)

  val remove : elt -> ?mult:int -> t -> t
  (** [remove x ?mult b] returns a new multiset where the multiplicity of [x]
      is decreased by [mult] (defaults to one).
      Raises [Invalid_argument] if [mult] is negative.*)

  val remove_all : elt -> t -> t
  (** [remove_all x b] returns a new multiset where the element [x] is removed. *)

  val merge : (elt -> int -> int -> int) -> t -> t -> t
  (** [merge f b1 b2] computes a multiset whose elements are a subset of
  ...
  (** Returns the list of all elements of the given multiset. Each element
  ...
  (** Returns the smallest element in the given multiset (with respect
      to the ordering) with its multiplicity,
      or raises [Not_found] if the multiset is empty. *)

  val min_elt_opt : t -> (elt * int) option
  (** Returns the smallest element of the given multiset
      (with respect to the ordering) with its multiplicity,
      or [None] if the multiset is empty. *)

  val max_elt : t -> elt * int
  (** Returns the largest element in the given multiset (with respect to
      the ordering) with its multiplicity, or raises [Not_found] if the
      multiset is empty. *)

  val max_elt_opt : t -> (elt * int) option
  (** Returns the largest element of the given multiset (with respect to
      the ordering) with its multiplicity, or [None] if the multiset is
      empty. *)

  val choose : t -> elt * int
  (** Returns one element of the given multiset with its multiplicity, or
      raises [Not_found] if the multiset is empty. Which binding is chosen
      is unspecified, but equal elements will be chosen for equal
      multisets. *)

  val choose_opt : t -> (elt * int) option
  (** Returns one element of the given multiset, or [None] if
      the set is empty. Which element is chosen is unspecified,
      but equal elements will be chosen for equal multisets. *)

  val union : t -> t -> t
  (** [union b1 b2] returns a new multiset [b] where, for all element x,
      [occ x b = max (occ x b1) (occ x b2)]. *)

  val sum : t -> t -> t
  (** [sum b1 b2] returns a new multiset [b] where, for all element x,
      [occ x b = occ x b1 + occ x b2]. *)

  val mul : t -> int -> t
  (** [mul b n] returns a new multiset [b'] where, for all element x,
      [occ x b' = occ x b * n].
  ...
  val inter : t -> t -> t
  (** [inter b1 b2] returns a new multiset [b] where, for all element x,
      [occ x b = min (occ x b1) (occ x b2)]. *)

  val diff : t -> t -> t
  (** [diff b1 b2] returns a new multiset [b] where, for all element x,
      [occ x b = max 0 (occ x b1 - occ x b2)]. *)

  val disjoint : t -> t -> bool
  (** Test if two multisets are disjoint. *)
  ...
  (** [iter f b] applies [f] to all elements in multiset [b].  [f] receives the
  ...
  (** [fold f b a] computes [(f xN mN ... (f x1 m1 a)...)] , where [x1 ... xN]
      are the elements in multiset [b] (in increasing order), and [m1 ... mN] are
      their multiplicities. *)

  val for_all : (elt -> int -> bool) -> t -> bool
  (** [for_all p b] checks if all the elements of the multiset satisfy the predicate
      [p]. *)

  val exists : (elt -> int -> bool) -> t -> bool
  (** [exists p b] checks if at least one element of the multiset satisfies the
      predicate [p]. *)

  val filter : (elt -> int -> bool) -> t -> t
  (** [filter p b] returns the multiset with all the elements in [b] that satisfy
       predicate [p]. Multiplicities are unchanged. *)

  val partition : (elt -> int -> bool) -> t -> t * t
  (** [partition p b] returns a pair of multisets [(b1, b2)], where
      [b1] contains all the elements of [b] that satisfy the
      predicate [p], and [b2] is the multiset with all the elements of
      [b] that do not satisfy [p]. *)

  val split : elt -> t -> t * int * t
  (** [split x b] returns a triple [(l, m, r)], where
        [l] is the multiset with all the elements of [b] that
      are strictly less than [x];
        [r] is the multiset with all the elements of [b] that
  ...
  val map : (int -> int) -> t -> t
  (** [map f b] returns a multiset with same elements as [b], where the
  ...
  (** Same as {!Multiset.map}, but the function receives as arguments both the
  ...
  val equal : t -> t -> bool
  (** [equal b1 b2] tests whether the multisets [b1] and [b2] are equal, that is,
      contain equal elements with equal multiplicities. *)

  (** {1 Iterators} *)

  val to_seq : t -> (elt * int) Seq.t
  (** Iterates on the whole multiset, in ascending order of elements. *)
  ...
  (** Adds the given elements to the multiset, in order.
      Raises [Invalid_argument] if a multiplicity is negative. *)

  val of_seq : (elt * int) Seq.t -> t
  (** Builds a multiset from the given elements and multiplicities.
      Raises [Invalid_argument] if a multiplicity is negative. *)

  val add_list : (elt * int) List.t -> t -> t
  (** Adds the given elements to the multiset, in order.
      Raises [Invalid_argument] if a multiplicity is negative. *)

  val of_list : (elt * int) List.t -> t
  (** Builds a multiset from the given elements and multiplicities.
      Raises [Invalid_argument] if a multiplicity is negative. *)

  val pp : (Format.formatter -> X.t -> unit) -> Format.formatter -> t -> unit
end
