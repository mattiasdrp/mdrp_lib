...
type 'a bt = { rank : int; root : 'a; kids : 'a bt list }
(** binomial trees *)
...
let insert bh x =
  ...
  let mind =
    match bh.mind with None -> Some x | Some mind -> Some (min x mind)
  in
  { size; data; mind }

(*$
  T size;
  empty size (insert empty 3) = 1 size empty = 0
*)
...
(*$
  T find_min;
  insert;
  empty find_min (insert (insert empty 3) 5)
  = 3 find_min (insert (insert empty 5) 3)
  = 3
*)
...
let rec find_min_tree ts ~kfail ~ksuccess =
  ...
  | t :: ts ->
      find_min_tree ts ~kfail ~ksuccess:(fun u ->
          if Stdlib.compare t.root u.root <= 0 then ksuccess t else ksuccess u)
...
let rec del_min_tree bts ~kfail ~ksuccess =
  ...
  | t :: ts ->
      del_min_tree ts ~kfail ~ksuccess:(fun u uts ->
          if Stdlib.compare t.root u.root <= 0 then ksuccess t ts
...
(*$
  T to_list;
  empty to_list (insert (insert empty 4) 6)
  = [ 4; 6 ] to_list (insert (insert empty 6) 4)
  = [ 4; 6 ] to_list empty
  = []
*)
...
(*$
  Q to_list;
  insert;
  empty (Q.list Q.int) ~count:10 (fun l ->
      to_list (List.fold_left insert empty l) = List.sort Stdlib.compare l)
*)
